...
export function leverForest(nodes,edges){
 ...
 const allowed=edges.filter(e=>byId.has(e.from)&&byId.has(e.to)&&e.from!==e.to&&byId.get(e.from).branch===byId.get(e.to).branch&&['including','consist of','supported by','have example'].includes(e.type));
...
export function layoutLeverTree(nodes,edges,{x=24,y=0,width=1152,columns=4}={}){
 ...
 let cursor=y;for(const root of tree.roots){cursor=branch(root.id,x,cursor,width)+16;}
