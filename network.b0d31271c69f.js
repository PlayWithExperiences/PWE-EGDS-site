...
import {mapFullscreen} from './map-fullscreen.b0d31271c69f.js';
import {relationName,hierarchyTypes} from './content-model.b0d31271c69f.js';
import {mapCamera} from './map-camera.b0d31271c69f.js';
...
export function graphData(catalog, selected='', scope='all'){
 ...
 if(scope==='focus'&&catalog.entries.some(n=>n.id===selected)){
  ...
  for(let step=0;step<(byId.get(selected)?.category==='levers'?1:2);step++){
  ...
  for(let step=0;step<3;step++)for(const e of edges)if(e.type==='supported by'&&visible.has(e.from)&&byId.get(e.from)?.name==='游戏系统 game systems')visible.add(e.to);
