import {bindImageViewer} from './image-viewer.b0d31271c69f.js';
import {drawingPanels,bindDrawings} from './drawing-viewer.b0d31271c69f.js';
import {contentModel,hierarchyEdges,relationName} from './content-model.b0d31271c69f.js';
import {mountStructureMap} from './structure-map.b0d31271c69f.js';
import {valenceOf,displayTitle} from './feeling-groups.b0d31271c69f.js';
import {mountNetwork,graphData} from './network.b0d31271c69f.js';
import { marked } from './vendor/marked.b0d31271c69f.js';
import { text } from './copy.b0d31271c69f.js';
...
function bindTheme(){
 ...
 function update(){const dark=document.documentElement.dataset.theme==='dark';button.textContent=dark?'☀':'☾';button.setAttribute('aria-pressed',String(dark));button.setAttribute('aria-label',lang==='zh'?(dark?'切换浅色主题':'切换暗色主题'):(dark?'Switch to light theme':'Switch to dark theme'));button.title=button.getAttribute('aria-label');document.querySelector('meta[name="theme-color"]')?.setAttribute('content',dark?'#141b2b':'#f3f0e8');}
...
function nav(){
 ...
 $('#footer').innerHTML=`<div class="wrap footer-inner"><div><a class="logo" href="#/">EGDS ↗</a><p>${t().footer}</p></div><div class="footer-meta"><span>${lang==='zh'?'方法论 v'+catalog.methodology+' / 内容快照 '+catalog.snapshotDate:'METHODOLOGY v'+catalog.methodology+' / SNAPSHOT '+catalog.snapshotDate}</span><span>© 2026 無涘 · Play With Experiences</span></div></div>`;
}
...
function fillResults(params){
 ...
 let matches=[...[0,1,2,3].map(formNode),...catalog.entries].filter(n=>(!cat||n.category===cat)&&(!status||n.status===status)&&(!branchSet||branchSet.has(n.id))&&(!q||[n.name,n.title.zh,n.title.en,n.body,...(n.drawings||[]).map(d=>d.text)].join(' ').toLocaleLowerCase().includes(q)));
 ...
 $('#result-list').innerHTML=matches.length?matches.map(n=>link(href(n.id),`<span class="result-type">${categoryName(n.category)}</span><div><h2>${esc(title(n))}</h2><span class="entry-subtitle">${esc(lang==='zh'?n.title.en:n.title.zh)}</span>${n.body?`<p>${esc(plain(n.body).slice(0,105))}${plain(n.body).length>105?'…':''}</p>`:''}</div><span class="entry-status ${n.body?'has-body':''}">${statusLabel(n)}</span><span class="result-arrow" aria-hidden="true">↗</span>`,'result-row')).join(''):`<div class="empty"><h2>${t().noResults}</h2><p>${t().noResultsHint}</p>${link(atlasURL(),t().all+' →','text-link')}</div>`;
}
function bindAtlas(params){
 if(isStructure(params)){const expand=$('#expand-all');expand.onclick=()=>{const open=expand.getAttribute('aria-expanded')!=='true';document.querySelectorAll('.structure-layers details').forEach(el=>el.open=open);expand.setAttribute('aria-expanded',String(open));expand.textContent=lang==='zh'?(open?'收起全部节点':'展开全部节点'):(open?'Collapse all nodes':'Expand all nodes');if(open)$('.structure-layers').scrollIntoView({behavior:'smooth'});};mountStructureMap($('#structure-map'),{lang,onOpen:key=>{const branch=['gameplay','narrative','aesthetics'].includes(key);const layer=branch?'levers':key;document.querySelectorAll('.content-layer').forEach(el=>el.open=el.dataset.contentLayer===layer);const target=document.querySelector(`[data-content-layer="${layer}"]`);if(branch){const index=['gameplay','narrative','aesthetics'].indexOf(key);target.querySelectorAll('.lever-section').forEach((el,i)=>el.open=i===index);}target.scrollIntoView({behavior:'smooth',block:'start'});}});return;}
...
try{
 const response=await fetch('./data/catalog.b0d31271c69f.json');if(!response.ok)throw new Error(`Catalog HTTP ${response.status}`);catalog=contentModel(await response.json());
